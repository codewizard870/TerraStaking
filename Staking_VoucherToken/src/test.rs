...
//deposit
    ...
    info.funds = vec![Coin{denom: "uusd".to_string(), amount: Uint128::from(1000_000_000u128)}];
    seconds += MONTH;
    env.block.time = Timestamp::from_seconds(seconds.clone());
    let msg = ExecuteMsg::DepositUst { qualified: true };
...
//deposit
    info.sender = Addr::unchecked("user1".to_string());
    info.funds = vec![Coin{denom: "uluna".to_string(), amount: Uint128::from(200_000_000u128)}];
    seconds += MONTH;
    env.block.time = Timestamp::from_seconds(seconds.clone());
    let msg = ExecuteMsg::DepositLuna { qualified: false };
    ...
    let res = try_withdraw_luna(deps.as_mut(), env.clone(), Addr::unchecked("user1".to_string()),
        Uint128::from(100_000_000u128), Uint128::from(100u128), Uint128::from(8000u128)).unwrap();

    println!("withdraw {:?}\n", res);

//Amount History
    let msg = QueryMsg::GetAmountHistory{};
    let res = query(deps.as_ref(), env.clone(), msg).unwrap();
    let res: Vec<AmountInfo> = from_binary(&res).unwrap();
    println!("Get Amount History {:?}\n", res );
//rewards
    info.sender = Addr::unchecked("treasury".to_string());
    let msg = ExecuteMsg::RewardsUst{ };
    let res = execute(deps.as_mut(), env.clone(), info.clone(), msg).unwrap();
    println!("rewards{:?}\n", res);

//farm
    info.sender = Addr::unchecked("treasury".to_string());
    let msg = ExecuteMsg::Farm{ust_price: Uint128::from(100u128), luna_price: Uint128::from(8000u128) };
    let res = execute(deps.as_mut(), env.clone(), info.clone(), msg).unwrap();
    println!("Farm {:?}\n", res);
...
//withdraw
    info.sender = Addr::unchecked("treasury".to_string());
    let res = try_withdraw_ust(deps.as_mut(), env.clone(), Addr::unchecked("user1".to_string()),
        Uint128::from(500_000_000u128), Uint128::from(100u128), Uint128::from(8000u128)).unwrap();
